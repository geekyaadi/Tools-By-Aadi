...
class TBA_Ajax {
    ...
    public static function init() {
        $actions = [
            ...
            'tba_check_duplicate',
            'tba_get_step_status',
            'tba_save_reference_image',
            'tba_delete_reference_image',
            'tba_ping_key',
            'tba_ping_all_keys',
            'tba_generate_planner_titles',
            'tba_save_planner_tasks',
            'tba_process_queue_item',
            'tba_generate_pending_thumbnail',
            'tba_generate_tags',
            'tba_translate_post',
            'tba_request_indexing',
            'tba_rewrite_post',
        ];
    ...
    private static function verify_nonce() {
        if ( ! check_ajax_referer( 'tba_nonce', 'nonce', false ) ) {
            self::error( __( 'Security check failed.', 'tools-by-aadi' ) );
            exit;
        }
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( __( 'Unauthorized capability check failed.', 'tools-by-aadi' ) );
            exit;
        }
    }
    ...
    public static function handle_tba_get_titles() {
        self::verify_nonce();
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $session_id     = sanitize_text_field( wp_unslash( $_POST['session_id'] ?? uniqid( 'tba_'  ) ) );

        if ( empty( $niche ) ) {
            self::error( __( 'Please enter a niche.', 'tools-by-aadi' ) );
            return;
        }

        $result = TBA_Gemini::get_title_suggestions( $session_id, $niche, $focus_keywords );
        ...
        self::success( [
            ...
            'key_used'   => TBA_Key_Manager::mask_key( $result['key_used'] ?? '' ),
    ...
    public static function handle_tba_generate_post() {
        self::verify_nonce();
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $post_status    = sanitize_text_field( wp_unslash( $_POST['post_status'] ?? get_option( 'tba_default_status', 'draft'  ) ) );
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $skip_publish   = (bool) ( $_POST['preview_only'] ?? false );

        if ( empty( $session_id ) || empty( $title ) ) {
            self::error( __( 'Missing session or title.', 'tools-by-aadi' ) );
            return;
        }

        // Review Mode: always force draft
        if ( get_option( 'tba_review_mode', 0 ) ) {
            $post_status = 'draft';
        }
        ...
        switch ( $step ) {
            ...
            // ------------------------------------------------------------------
            case 'article':
                $result = TBA_Gemini::generate_article( $session_id, $title, $focus_keywords );
            ...
            case 'tags':
                ...
                $result = TBA_Gemini::generate_tags( $session_id, $title, 'English', $tag_count );
            ...
            // ------------------------------------------------------------------
            case 'meta':
                $result = TBA_Gemini::generate_meta_description( $session_id, $title, $focus_keywords );
            ...
            case 'category':
                ...
                $result = TBA_Gemini::suggest_category( $session_id, $title );
            ...
            case 'thumbnail':
                ...
                $result = TBA_Gemini::generate_thumbnail( $session_id, $title, $ref_image, $t2i_opts );
                ...
                self::success( [
                    ...
                    'cached'         => $result['cached'],
                    'key_used'       => $result['key_used'] ? TBA_Key_Manager::mask_key( $result['key_used'] ) : '',
                    'switched'       => $result['switched'] ?? false,
                ] );
                break;
            ...
            case 'og_image':
                ...
                $result = TBA_Gemini::generate_og_image( $session_id, $title, $t2i_opts );
            ...
            // ------------------------------------------------------------------
            case 'alt_text':
                $result = TBA_Gemini::generate_alt_text( $session_id, $title );
            ...
            case 'publish':
                ...
                $article      = TBA_Gemini::get_cached_step( $session_id, 'article' );
                $tags         = TBA_Gemini::get_cached_step( $session_id, 'tags' );
                $meta         = TBA_Gemini::get_cached_step( $session_id, 'meta' );
                $category     = TBA_Gemini::get_cached_step( $session_id, 'cat' );
                $thumb_data   = TBA_Gemini::get_cached_step( $session_id, 'thumb' );
                $og_data      = TBA_Gemini::get_cached_step( $session_id, 'og' );
                $alt_text     = TBA_Gemini::get_cached_step( $session_id, 'alt' );

                if ( ! $article ) {
                    self::error( __( 'Article data not found. Please regenerate.', 'tools-by-aadi' ) );
                    return;
                }
                ...
                // Duplicate check
                $dup = TBA_Duplicate_Check::find_duplicate( $title );
                if ( $dup ) {
                    self::success( [
                ...
                $post_id = TBA_Post_Creator::create( [
                    'title'            => $title,
                    'article'          => $article,
                    'tags'             => $tags ?: [],
                ...
                // Log to history
                $token_est = TBA_History::estimate_tokens(
                    $article,
                    is_array( $tags ) ? implode( ',', $tags ) : '',
                    $meta ?: ''
                );
                $history_id = TBA_History::insert( [
                    'session_id'     => $session_id,
                    'niche'          => $niche,
                    'title'          => $title,
                    'post_id'        => $post_id,
                    'status'         => 'success',
                    'key_used'       => TBA_Gemini::get_current_key( $session_id ) ?? '',
                    'token_estimate' => $token_est,
                ] );

                TBA_Gemini::clear_session_cache( $session_id );
            ...
            default:
                self::error( __( 'Unknown generation step.', 'tools-by-aadi' ) );
        }
    }
    ...
    public static function handle_tba_check_duplicate() {
        self::verify_nonce();
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $title = sanitize_text_field( wp_unslash( $_POST['title'] ?? ''  ) );
        $dup   = TBA_Duplicate_Check::find_duplicate( $title );
    ...
    public static function handle_tba_get_step_status() {
        self::verify_nonce();
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $session_id = sanitize_text_field( wp_unslash( $_POST['session_id'] ?? ''  ) );
        ...
        $done  = [];
        foreach ( $steps as $s ) {
            if ( TBA_Gemini::get_cached_step( $session_id, $s ) ) {
                $done[] = $s;
            }
        }
        ...
        self::success( [
            ...
            'current_key'     => TBA_Key_Manager::mask_key( TBA_Gemini::get_current_key( $session_id ) ?? '' ),
        ] );
    }
    ...
    public static function handle_tba_save_reference_image() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
        ...
        TBA_Gemini::save_default_reference_image( $base64, $mime_type );
    ...
    public static function handle_tba_delete_reference_image() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        TBA_Gemini::delete_default_reference_image();
    ...
    public static function handle_tba_ping_key() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $result  = TBA_Key_Manager::ping_key( $api_key );
        ...
        TBA_Key_Manager::save_all_keys( $keys );
        ...
        self::success( [
            ...
            'reset_in_human' => $reset_in_secs !== null
                                    ? TBA_Key_Manager::format_seconds( $reset_in_secs )
                                    : null,
        ] );
    }
    ...
    public static function handle_tba_ping_all_keys() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $raw_results = TBA_Key_Manager::ping_all_keys();
        $keys        = TBA_Key_Manager::get_all_keys();
        ...
        $stats = TBA_Key_Manager::get_stats();
        self::success( [
            'results' => $formatted,
            'summary' => [
                'active'    => $stats['active'],
    ...
    public static function handle_tba_generate_planner_titles() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        if ( $mode === 'silo' ) {
            $result = TBA_Gemini::get_silo_suggestions( $session_id, $niche, $language );
            if ( is_wp_error( $result ) ) {
                self::error( $result->get_error_message() );
                return;
            }
        ...
        $result = TBA_Gemini::get_planner_title_suggestions( $session_id, $niche, $language, $count );
        ...
        if ( empty( $titles ) ) {
            self::error( __( 'No titles could be parsed. Try again.', 'tools-by-aadi' ) );
            return;
        }
    ...
    public static function handle_tba_save_planner_tasks() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        if ( empty( $niche ) || empty( $tasks ) ) {
            self::error( __( 'Missing niche or task list.', 'tools-by-aadi' ) );
            return;
        }
        ...
        if ( $mode === 'silo' ) {
            ...
            if ( $pillar_title ) {
                $pillar_queue_id = TBA_Queue::enqueue( $niche, $pillar_title, $language, $pillar_cat, 0, null, 'pillar', $tag_count, $meta );
                if ( $pillar_queue_id ) {
                    // Update it to have silo_id = itself
                    global $wpdb;
                    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                    $wpdb->update( TBA_Queue::table_name(), [ 'silo_id' => $pillar_queue_id ], [ 'id' => $pillar_queue_id ] );
                    $count++;
                    ...
                    foreach ( $tasks as $task ) {
                        ...
                        if ( $title ) {
                            TBA_Queue::enqueue( $niche, $title, $language, $category, 0, $pillar_queue_id, 'cluster', $tag_count, $meta );
                            $count++;
                        }
                    }
                }
            ...
            foreach ( $tasks as $task ) {
                ...
                if ( $title ) {
                    TBA_Queue::enqueue( $niche, $title, $language, $category, 0, null, null, $tag_count, $meta );
                    $count++;
                }
            }
        }
        ...
        self::success( [
            ...
            'message' => sprintf( __( 'Successfully enqueued %d tasks in the background worker.', 'tools-by-aadi' ), $count )
        ] );
    }
    ...
    public static function handle_tba_process_queue_item() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $stats = TBA_Key_Manager::get_stats();
        if ( $stats['active'] === 0 ) {
            self::error( __( 'All API keys are exhausted. Pausing queue processor.', 'tools-by-aadi' ) );
            return;
        }

        $next = TBA_Queue::get_next_queued();
        if ( ! $next ) {
            self::success( [ 'processed' => false, 'message' => __( 'Queue is empty.', 'tools-by-aadi' ) ] );
            return;
        }

        // Process item
        TBA_Queue::mark_processing( $next->id );
        ...
        if ( ! empty( $next->post_id ) ) {
            ...
            if ( is_wp_error( $res ) ) {
                TBA_Queue::mark_failed( $next->id, $res->get_error_message() );
                self::error( $res->get_error_message() );
                return;
            }
            TBA_Queue::mark_published( $next->id, $next->post_id );
        ...
        $post_id = TBA_Scheduler::generate_and_publish(
            $next->niche,
            $next->title,
            '',
        ...
        if ( is_wp_error( $post_id ) ) {
            TBA_Queue::mark_failed( $next->id, $post_id->get_error_message() );
            self::error( $post_id->get_error_message(), [ 'id' => $next->id ] );
            return;
        }

        TBA_Queue::mark_published( $next->id, $post_id );
        self::success( [
            'processed' => true,
            'id'        => $next->id,
            'title'     => $next->title,
    ...
    public static function handle_tba_generate_pending_thumbnail() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        if ( ! in_array( $engine, [ 'ai', 'text_to_image' ], true ) ) {
            $engine = get_option( 'tba_thumb_type', 'ai' );
        }

        $session_id = 'thumb_regen_' . uniqid();
        $result = TBA_Gemini::generate_thumbnail( $session_id, $post->post_title, [], [ 'thumb_type' => $engine ] );
        ...
        $attachment_id = TBA_Post_Creator::upload_image(
            $image_data['base64'],
        ...
        delete_post_meta( $post_id, '_tba_thumbnail_pending' );
    ...
    public static function handle_tba_generate_tags() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $result = TBA_Gemini::generate_tags( $session_id, $post->post_title, 'English', $tag_count );
    ...
    public static function handle_tba_translate_post() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        // 1. Translate Title
        $title_res = TBA_Gemini::translate_text( $session_id, $post->post_title, $target_lang );
        ...
        $article_res = TBA_Gemini::translate_html( $session_id, $post->post_content, $target_lang );
        ...
        if ( empty( $orig_meta ) ) {
            $orig_meta = get_post_meta( $post_id, '_tba_meta_description', true );
        }
        
        $translated_meta = '';
        if ( ! empty( $orig_meta ) ) {
            $meta_res = TBA_Gemini::translate_text( $session_id, $orig_meta, $target_lang );
        ...
        if ( ! empty( $orig_tags ) ) {
            ...
            $tags_res = TBA_Gemini::translate_text( $session_id, $tags_string, $target_lang );
        ...
        // Meta flags
        update_post_meta( $new_post_id, '_tba_generated', '1' );
        update_post_meta( $new_post_id, '_tba_translated_from', $post_id );
        update_post_meta( $new_post_id, '_tba_translation_lang', $target_lang );
        ...
        if ( ! empty( $translated_meta ) ) {
            update_post_meta( $new_post_id, '_tba_meta_description', $translated_meta );
    ...
    public static function handle_tba_request_indexing() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $url    = get_permalink( $post_id );
        $result = TBA_GSC_Helper::submit_url( $url, 'URL_UPDATED' );
        ...
        update_post_meta( $post_id, '_tba_gsc_last_ping', current_time( 'mysql' ) );
    ...
    public static function handle_tba_rewrite_post() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
...
\"\"\"";
        ...
        $result = TBA_Gemini::request( $session_id, TBA_Gemini::get_text_model(), [
            'contents' => [
                [ 'parts' => [ [ 'text' => $prompt ] ] ]
            ],
            'generationConfig' => [
        ...
        $rewritten = TBA_Gemini::extract_text( $result['data'] );
