...
class TBA_Rate_Limits {
    ...
    public static function get_usage( string $hash ) {
        return [
            'rpm' => (int) ( get_transient( 'tba_rpm_' . $hash ) ?: 0 ),
            'tpm' => (int) ( get_transient( 'tba_tpm_' . $hash ) ?: 0 ),
            'rpd' => (int) ( get_transient( 'tba_rpd_' . $hash . '_' . self::today_key() ) ?: 0 ),
        ];
    }
    ...
    public static function reset_counters( string $api_key ) {
        $hash = self::key_hash( $api_key );
        delete_transient( 'tba_rpm_' . $hash );
        delete_transient( 'tba_tpm_' . $hash );
        delete_transient( 'tba_rpd_' . $hash . '_' . self::today_key() );
    }
    ...
    private static function increment_rpd( string $hash ) {
        $key = 'tba_rpd_' . $hash . '_' . self::today_key();
        $val = (int) get_transient( $key );
