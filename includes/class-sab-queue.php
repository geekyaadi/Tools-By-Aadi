 ...
 * Works with SAB_Scheduler for timed publishing.
 */
...
class SAB_Queue {
    ...
    public static function get_next_queued() {
        ...
        if ( get_transient( 'sab_queue_lock' ) ) {
            return null;
        }
        ...
        return $wpdb->get_row( "SELECT * FROM {$table} WHERE status = 'queued' ORDER BY created_at ASC LIMIT 1" );
    }
    ...
    public static function mark_processing( int $id ) {
        ...
        set_transient( 'sab_queue_lock', $id, 900 ); // Lock the queue for 15 minutes
        set_transient( 'sab_task_processing_' . $id, time(), 900 ); // Track this specific task
    }
    ...
    public static function mark_published( int $id, int $post_id ) {
        ...
        delete_transient( 'sab_queue_lock' );
        delete_transient( 'sab_task_processing_' . $id );
    }
    ...
    public static function recover_stuck_tasks() {
        ...
        if ( ! empty( $processing ) ) {
            foreach ( $processing as $item ) {
                ...
                if ( ! get_transient( 'sab_task_processing_' . $item->id ) ) {
                    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
                    $wpdb->update( $table, [
                        'status'    => 'failed',
                        'error_msg' => __( 'Process timed out or crashed during execution.', 'soniji-auto-blogging' ),
                    ], [ 'id' => $item->id ] );
                    
                    // Also clear global queue lock if it belonged to this task
                    $lock_id = get_transient( 'sab_queue_lock' );
                    if ( $lock_id == $item->id ) {
                        delete_transient( 'sab_queue_lock' );
                    }
                }
    ...
    public static function delete_multiple( array $ids ) {
        ...
        $lock_id = (int) get_transient( 'sab_queue_lock' );
