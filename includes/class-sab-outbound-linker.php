...
class SAB_Outbound_Linker {
    ...
    public static function process( $content, $title = '' ) {
        if ( get_option( 'sab_enable_outbound_linking', '1' ) !== '1' ) {
            return $content;
        }

        $max_links  = max( 1, min( 10, intval( get_option( 'sab_max_outbound_links', 2 ) ) ) );
        $target     = sanitize_text_field( get_option( 'sab_outbound_target', '_blank' ) );
        $rel        = sanitize_text_field( get_option( 'sab_outbound_rel', 'nofollow noopener' ) );
        $blacklist  = array_filter( array_map( 'trim', explode( "\n", strtolower( get_option( 'sab_outbound_blacklist', '' ) ) ) ) );
        ...
        foreach ( self::$high_da_database as $keyword => $url ) {
            ...
            if ( preg_match( $pattern, $content ) ) {
                $link_html = sprintf(
                    '<a href="%s" target="%s" rel="%s" class="sab-outbound-link">%s</a>',
                    esc_url( $url ),
                    esc_attr( $target ),
                    esc_attr( $rel ),
                    '$1'
    ...
    private static function fetch_wikipedia_url( $query ) {
        $transient_key = 'sab_wiki_' . md5( strtolower( $query ) );
