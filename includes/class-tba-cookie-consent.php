<?php
/**
 * Cookie Consent Controller Engine for Tools By Aadi
 * Supports Optional Reject Button & Custom Text
 */

if ( ! defined( 'ABSPATH' ) ) exit;

class TBA_Cookie_Consent {

    public static function init() {
        if ( get_option( 'tba_cookie_enable', '1' ) === '1' ) {
    ...
    public static function render_cookie_consent() {
        ...
        $style         = get_option( 'tba_cookie_style', 'bottom_banner' );
        $text          = get_option( 'tba_cookie_text', 'We use cookies to personalize content, ads, and analyze traffic for maximum user experience.' );
        $btn_text      = get_option( 'tba_cookie_btn_text', 'Accept All Cookies 🍪' );
        $show_reject   = get_option( 'tba_cookie_enable_reject', '0' ) === '1';
        $reject_text   = get_option( 'tba_cookie_reject_btn_text', 'Decline Non-Essential' );
        $privacy_url   = get_option( 'tba_cookie_privacy_url', home_url('/privacy-policy/') );
        ...
        ?>
        <div id="tba-cookie-banner" style="<?php echo esc_attr($css); ?>">
        ...
        function aapAcceptCookies() {
            document.cookie = "tba_cookie_consent=accepted; max-age=" + (365*24*60*60) + "; path=/";
            var banner = document.getElementById("tba-cookie-banner");
            if (banner) banner.style.display = "none";
        }
        function aapDismissCookies() {
            document.cookie = "tba_cookie_consent=declined; max-age=" + (30*24*60*60) + "; path=/";
            var banner = document.getElementById("tba-cookie-banner");
            if (banner) banner.style.display = "none";
        }
        (function() {
            if (document.cookie.indexOf("tba_cookie_consent=") !== -1) {
                var banner = document.getElementById("tba-cookie-banner");
                if (banner) banner.style.display = "none";
            }
        })();
