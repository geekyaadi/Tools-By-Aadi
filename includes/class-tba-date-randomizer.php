<?php
/**
 * Post Date Randomizer & Backdater Engine for Tools By Aadi
 * Randomizes post publishing dates and comment timestamps within a custom date range.
 */

if ( ! defined( 'ABSPATH' ) ) exit;

class TBA_Date_Randomizer {

    public static function init() {
        add_action( 'admin_post_tba_save_randomizer_settings', [ __CLASS__, 'handle_save_settings' ] );
        add_action( 'admin_post_tba_run_randomizer',           [ __CLASS__, 'handle_run_randomizer' ] );
    }

    public static function handle_save_settings() {
        if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Unauthorized' );
        check_admin_referer( 'tba_randomizer_nonce' );
        ...
        update_option( 'tba_randomizer_start_date', $start_date );
        update_option( 'tba_randomizer_end_date',   $end_date );
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        update_option( 'tba_randomizer_posts',        isset( $_POST['randomize_posts'] ) ? '1' : '0' );
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        update_option( 'tba_randomizer_comments',     isset( $_POST['randomize_comments'] ) ? '1' : '0' );
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        update_option( 'tba_randomizer_post_type',    sanitize_text_field( $_POST['post_type'] ?? 'post' ) );
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        update_option( 'tba_randomizer_modified_date',isset( $_POST['set_modified_date'] ) ? '1' : '0' );

        wp_safe_redirect( admin_url( 'admin.php?page=tba-randomizer&saved=true' ) );
        exit;
    }
    ...
    public static function handle_run_randomizer() {
        ...
        $start_str = get_option( 'tba_randomizer_start_date', '' );
        $end_str   = get_option( 'tba_randomizer_end_date', '' );
        ...
        if ( ! $start_ts || ! $end_ts || $start_ts >= $end_ts ) {
            wp_safe_redirect( admin_url( 'admin.php?page=tba-randomizer&error=invalid_range' ) );
            exit;
        }

        $randomize_posts    = get_option( 'tba_randomizer_posts', '1' ) === '1';
        $randomize_comments = get_option( 'tba_randomizer_comments', '1' ) === '1';
        $post_type          = get_option( 'tba_randomizer_post_type', 'post' );
        $set_modified       = get_option( 'tba_randomizer_modified_date', '1' ) === '1';
        ...
        wp_safe_redirect( admin_url( 'admin.php?page=tba-randomizer&run=success&count=' . $affected_count ) );
        exit;
    }
}
