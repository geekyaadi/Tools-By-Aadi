...
class TBA_Gemini {
    ...
    public static function get_text_model() {
        $provider = get_option( 'tba_active_provider', 'gemini' );
        if ( $provider === 'openai' ) {
            return get_option( 'tba_openai_model', 'gpt-4o-mini' );
        }
        return get_option( 'tba_text_model', self::DEFAULT_TEXT_MODEL );
    }
    ...
    public static function get_image_model() {
        ...
        $model = get_option( 'tba_image_model', self::DEFAULT_IMAGE_MODEL );
        if ( $model === 'gemini-2.0-flash-preview-image-generation' || $model === 'gemini-3.1-flash-image-generation-preview' ) {
            ...
            update_option( 'tba_image_model', $model );
        }
        return $model;
    }
    ...
    public static function get_current_key( string $session_id ) {
        $key = get_transient( 'tba_session_key_' . $session_id );
        if ( ! $key ) {
            $key = TBA_Key_Manager::get_active_key();
            if ( $key ) {
                set_transient( 'tba_session_key_' . $session_id, $key, self::CACHE_TTL );
            }
        }
        return $key;
    }
    ...
    public static function request( string $session_id, string $model, array $body ) {
        ...
        $active_provider    = get_option( 'tba_active_provider', 'gemini' );
        ...
        if ( empty( $primary_keys ) && empty( $secondary_keys ) ) {
            return new WP_Error( 'no_key', __( 'No active API keys available.', 'tools-by-aadi' ) );
        }

        // Determine model type
        $image_model_names = [ 'gemini-2.0-flash', 'gemini-2.0-flash-exp', 'dall-e-3', 'imagen-3.0-generate-002' ];
        if ( isset( TBA_Rate_Limits::MODELS[ $model ] ) ) {
            $model_type = TBA_Rate_Limits::MODELS[ $model ]['type'] ?? 'text';
        ...
        $primary_models = TBA_Rate_Limits::get_fallback_models_for_type( $model_type, $active_provider );
        $primary_models = array_values( array_unique( array_merge( [ $model ], $primary_models ) ) );

        $secondary_models = TBA_Rate_Limits::get_fallback_models_for_type( $model_type, $secondary_provider );
        ...
        $slot_transient = 'tba_session_slot_' . $session_id . '_' . $model_type;
        ...
        while ( true ) {
            ...
            TBA_Key_Manager::auto_reset_check();

            // Refresh key status
            $key_data = null;
            foreach ( TBA_Key_Manager::get_all_keys() as $k ) {
            ...
            if ( ! TBA_Rate_Limits::is_within_limits( $current_key, $current_model ) ) {
                $tried_set[ $current_slot ] = true;
                if ( count( $tried_set ) >= $total_slots ) break;
                $current_slot = ( $current_slot + 1 ) % $total_slots;
            ...
            TBA_Key_Manager::increment_requests( $current_key );
            ...
            if ( $http_code === 429 ||
                 ...
                 ( $slot_provider === 'gemini' && TBA_Key_Manager::is_quota_error( $response ) ) ) {

                $retry_after = TBA_Key_Manager::parse_retry_after( $response, $body_decoded );
                ...
                if ( ! $key_has_untried ) {
                    TBA_Key_Manager::mark_exhausted( $current_key, $retry_after );
                }
            ...
            if ( in_array( $http_code, [ 400, 401, 403 ], true ) ) {
                ...
                elseif ( $slot_provider === 'gemini' && TBA_Key_Manager::is_invalid_key_error( $response ) ) $is_invalid = true;

                if ( $is_invalid ) {
                    TBA_Key_Manager::mark_invalid( $current_key );
            ...
            if ( $token_est > 0 ) {
                TBA_Key_Manager::increment_requests( $current_key, $token_est );
            }

            TBA_Rate_Limits::record( $current_key, $current_model, $token_est );
        ...
        return new WP_Error( 'all_exhausted', __( 'All API key + model combinations are exhausted. Please wait for quota reset or add more keys.', 'tools-by-aadi' ) );
    }
    ...
    public static function get_title_suggestions( string $session_id, string $niche, string $focus_keywords = '', string $language = 'English' ) {
        $cache_key = 'tba_titles_' . $session_id;
...
Return ONLY a numbered list (1. Title, 2. Title, etc.) with no additional conversational text or formatting.";
        ...
        $prompt = self::get_custom_prompt( 'tba_prompt_titles', $default_prompt, [
            'count'           => 5,
            'niche'           => $niche,
            'language'        => $language,
...
Return ONLY a JSON array of strings, e.g. [\"Comment 1\", \"Comment 2\"]. No conversational text.";
    ...
    public static function get_article( string $session_id, string $title, string $focus_keywords = '', string $language = 'English' ) {
        $cache_key = 'tba_article_' . $session_id;
        $cached    = get_transient( $cache_key );
        if ( $cached ) return [ 'article' => $cached, 'cached' => true, 'switched' => false ];

        $word_count = (int) get_option( 'tba_word_count', 1000 );
        $tone       = get_option( 'tba_content_tone', 'professional' );
...
6. HTML FORMATTING: Use clean HTML tags (h2, h3, h4, p, table, thead, tbody, tr, th, td, ul, ol, li, strong, em). Paragraphs must be short (2-3 sentences max). Output ONLY the raw HTML content (no code block fences).";
        ...
        $prompt = self::get_custom_prompt( 'tba_prompt_article', $default_prompt, [
            'title'        => $title,
            'language'     => $language,
    ...
    public static function generate_tags( string $session_id, string $title, string $language = 'English', int $tag_count = 0 ) {
        ...
        if ( $tag_count <= 0 ) {
            $tag_count = (int) get_option( 'tba_tag_count', 15 );
        }
        $tag_count = max( 1, min( 100, $tag_count ) );

        $cache_key = 'tba_tags_' . $session_id . '_' . $tag_count;
        $cached    = get_transient( $cache_key );
...
Return ONLY a comma-separated list of tags, no numbering, no extra text.";
        ...
        $prompt = self::get_custom_prompt( 'tba_prompt_tags', $default_prompt, [
            'tag_count' => $tag_count,
            'title'     => $title,
            'language'  => $language,
        ] );
    ...
    public static function generate_meta_description( string $session_id, string $title, string $focus_keywords = '', string $language = 'English' ) {
        $cache_key = 'tba_meta_' . $session_id;
        $cached    = get_transient( $cache_key );
...
Make it compelling, high CTR, and write in a natural human tone. Return ONLY the meta description text, no quotes, no extra remarks.";
        ...
        $prompt = self::get_custom_prompt( 'tba_prompt_meta', $default_prompt, [
            'title'        => $title,
            'language'     => $language,
            'focus_clause' => $focus_clause,
            'keywords'     => $focus_keywords,
...
Return ONLY the category name, nothing else.";
    ...
    public static function generate_thumbnail( string $session_id, string $title, array $reference_image = [], array $t2i_opts = [] ) {
        $cache_key = 'tba_thumb_' . $session_id;
        $cached    = get_transient( $cache_key );
        if ( $cached ) return [ 'image_data' => $cached, 'cached' => true, 'switched' => false ];

        // Check if Title to Image (local GD) is selected
        $thumb_type = $t2i_opts['thumb_type'] ?? get_option( 'tba_thumb_type', 'ai' );
        if ( $thumb_type === 'text_to_image' ) {
            $bg_type = $t2i_opts['bg_type'] ?? get_option( 'tba_t2i_bg_type', 'gradient' );
            $bg_val  = $t2i_opts['bg_val']  ?? get_option( 'tba_t2i_bg_val', 'blue_purple' );
            $size    = $t2i_opts['size']    ?? get_option( 'tba_t2i_size', '600x315' );

            $t2i_result = TBA_Text_To_Image::generate( $title, $bg_type, $bg_val, $size );
            if ( ! $t2i_result ) {
                return new WP_Error( 't2i_failed', __( 'GD Text-to-Image generation failed.', 'tools-by-aadi' ) );
            }
...
Use vibrant colors, modern design, and include relevant visual elements. Make it suitable for a blog header.";
        ...
        if ( ! $image_data ) {
            return new WP_Error( 'no_image', __( 'No image returned by Gemini.', 'tools-by-aadi' ) );
        }
    ...
    public static function generate_og_image( string $session_id, string $title, array $t2i_opts = [] ) {
        ...
        if ( $thumb_type === 'text_to_image' ) {
            ...
            $t2i_result = TBA_Text_To_Image::generate( $title, $bg_type, $bg_val, '1200x630' );
            if ( ! $t2i_result ) {
                return new WP_Error( 't2i_failed', __( 'GD Text-to-Image OG generation failed.', 'tools-by-aadi' ) );
            }
...
Include the blog title text prominently, use a clean modern design with complementary colors.";
        ...
        $image_data = self::extract_image_data( $result['data'] );
        if ( ! $image_data ) {
            return new WP_Error( 'no_og_image', __( 'No OG image returned by Gemini.', 'tools-by-aadi' ) );
        }
...
Return ONLY the alt text, nothing else.";
    ...
    public static function clear_session_cache( string $session_id ) {
        ...
        foreach ( $keys as $k ) {
            delete_transient( 'tba_' . $k . '_' . $session_id );
        }
        delete_transient( 'tba_session_key_' . $session_id );
    }

    public static function get_cached_step( string $session_id, string $step ) {
        return get_transient( 'tba_' . $step . '_' . $session_id );
    }
    ...
    private static function get_blacklist_phrase() {
        $blacklist = get_option( 'tba_blacklist_words', '' );
    ...
    public static function get_default_reference_image() {
        $saved = get_option( 'tba_default_reference_image', [] );
    ...
    public static function save_default_reference_image( string $base64, string $mime_type ) {
        update_option( 'tba_default_reference_image', [
            'base64'    => $base64,
            'mime_type' => $mime_type,
        ] );
    }
    ...
    public static function delete_default_reference_image() {
        delete_option( 'tba_default_reference_image' );
    }
    ...
    public static function generate_faq( string $session_id, string $title, string $language = 'English', int $faq_count = 3 ) {
        if ( $faq_count <= 0 ) {
            $faq_count = (int) get_option( 'tba_faq_count', 3 );
        }
        $faq_count = max( 1, min( 20, $faq_count ) );

        $cache_key = 'tba_faq_' . $session_id . '_' . $faq_count;
        $cached    = get_transient( $cache_key );
...
[
  {
    ...
    \"answer\": \"Smart gardening involves using technology like automated sensors and watering systems to grow...\"
        ...
        $prompt = self::get_custom_prompt( 'tba_prompt_faq', $default_prompt, [
            'faq_count' => $faq_count,
            'title'     => $title,
            'language'  => $language,
        ] );
