<?php
/**
 * PHP GD Text-to-Image Generator for Tools By Aadi
...
class TBA_Text_To_Image {
    ...
    private static function get_font_path() {
        ...
        $font = TBA_PLUGIN_DIR . 'admin/fonts/Poppins-Bold.ttf';
        if ( file_exists( $font ) && self::ttf_works( $font ) ) {
            return $font;
        }

        // Try bundled Roboto Bold
        $font = TBA_PLUGIN_DIR . 'admin/fonts/Roboto-Bold.ttf';
        if ( file_exists( $font ) && self::ttf_works( $font ) ) {
            return $font;
        }

        // Try bundled Outfit Bold (variable font — may render thin)
        $font = TBA_PLUGIN_DIR . 'admin/fonts/Outfit-Bold.ttf';
        if ( file_exists( $font ) && self::ttf_works( $font ) ) {
            return $font;
        }
    ...
    public static function generate( string $text, string $bg_type, string $bg_val, string $size_val = '1200x630' ) {
        ...
        if ( $bg_type === 'mix' ) {
            ...
            if ( file_exists( TBA_PLUGIN_DIR . 'admin/default-thumbnail.jpg' ) ) {
                $choices[] = 'image';
            }
        ...
        if ( $bg_type === 'image' ) {
            $bg_img_path = TBA_PLUGIN_DIR . 'admin/default-thumbnail.jpg';
            $loaded = false;
