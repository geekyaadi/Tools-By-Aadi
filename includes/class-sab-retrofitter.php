<?php
/**
 * Retrofit Old Posts Engine & Dynamic Shortcode Content Filter for Soniji Auto Blogging
 * Dynamic Non-Destructive Content Filter & Shortcode engine for TOC, Auto-Internal Links, and High-DA Outbound Links.
 */

if ( ! defined( 'ABSPATH' ) ) exit;

class SAB_Retrofitter {

    public static function init() {
        add_action( 'admin_post_sab_retrofit_old_posts', [ __CLASS__, 'handle_retrofit_old_posts' ] );
        add_filter( 'the_content',                       [ __CLASS__, 'filter_the_content' ], 12 );

        // Shortcodes registration
        add_shortcode( 'sab_toc',            [ __CLASS__, 'shortcode_toc' ] );
        add_shortcode( 'sab_internal_links', [ __CLASS__, 'shortcode_internal_links' ] );
        add_shortcode( 'sab_outbound_links', [ __CLASS__, 'shortcode_outbound_links' ] );
    }
    ...
    public static function filter_the_content( $content ) {
        ...
        if ( get_option( 'sab_enable_toc', 1 ) && strpos( $content, 'class="sab-toc"' ) === false ) {
            $content = self::inject_toc( $content );
        }

        // 2. Dynamic Auto-Internal Linking
        if ( get_option( 'sab_enable_internal_linking', 1 ) && strpos( $content, 'sab-related-' ) === false ) {
            if ( class_exists( 'SAB_Post_Creator' ) && method_exists( 'SAB_Post_Creator', 'apply_internal_linking' ) ) {
                $content = SAB_Post_Creator::apply_internal_linking( $post_id, $content );
            }
        }

        // 3. Dynamic High-DA Outbound Linking
        if ( get_option( 'sab_enable_outbound_linking', 1 ) && strpos( $content, 'sab-outbound-link' ) === false ) {
            if ( class_exists( 'SAB_Outbound_Linker' ) && method_exists( 'SAB_Outbound_Linker', 'process' ) ) {
                $content = SAB_Outbound_Linker::process( $content, get_the_title( $post_id ) );
            }
        }
    ...
    public static function inject_toc( $content ) {
        ...
        if ( strpos( $content, 'class="sab-toc"' ) !== false || strpos( $content, 'id="sab-toc"' ) !== false ) {
            return $content;
        }
        ...
        $default_state = get_option( 'sab_toc_default_state', 'open' );
        $is_closed     = ( $default_state === 'closed' );
        $list_display  = $is_closed ? 'none' : 'block';
        $btn_label     = $is_closed ? '[Show]' : '[Hide]';

        $toc_html  = "\n<div class=\"sab-toc\" style=\"background:#f8fafc; border:1px solid #cbd5e1; border-radius:8px; padding:16px 20px; margin:24px 0; font-family: system-ui, -apple-system, sans-serif;\">\n";
        ...
        $toc_html .= "    <button type=\"button\" class=\"sab-toc-toggle-btn\" onclick=\"var list=this.parentElement.nextElementSibling; if(list.style.display==='none'){list.style.display='block';this.innerText='[Hide]';}else{list.style.display='none';this.innerText='[Show]';}\" style=\"background:none; border:none; color:#0284c7; font-weight:700; font-size:13px; cursor:pointer; padding:0;\">" . esc_html( $btn_label ) . "</button>\n";
        $toc_html .= "  </div>\n";
        $toc_html .= "  <ul class=\"sab-toc-list\" style=\"margin:12px 0 0 0; padding-left:20px; list-style-type:disc; display:" . esc_attr( $list_display ) . ";\">\n";
    ...
    public static function shortcode_toc( $atts = [], $content = null ) {
        global $post;
        if ( ! $post ) return '';
        return wp_kses_post( self::inject_toc( $post->post_content ) );
    }

    /**
     * Shortcode: [sab_internal_links]
     */
    public static function shortcode_internal_links( $atts = [], $content = null ) {
        global $post;
        if ( ! $post ) return '';
        return wp_kses_post( SAB_Post_Creator::apply_internal_linking( $post->ID, $post->post_content ) );
    }

    /**
     * Shortcode: [sab_outbound_links]
     */
    public static function shortcode_outbound_links( $atts = [], $content = null ) {
        global $post;
        if ( ! $post ) return '';
        return wp_kses_post( SAB_Outbound_Linker::process( $post->post_content, $post->post_title ) );
    }
    ...
    public static function process_single_post( $post_id, $do_internal = true, $do_outbound = true, $do_toc = true ) {
        ...
        update_post_meta( $post_id, '_sab_retrofit_toc',      $do_toc ? '1' : '0' );
        update_post_meta( $post_id, '_sab_retrofit_internal', $do_internal ? '1' : '0' );
        update_post_meta( $post_id, '_sab_retrofit_outbound', $do_outbound ? '1' : '0' );
        return true;
    }

    public static function handle_retrofit_old_posts() {
        if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Unauthorized' );
        check_admin_referer( 'sab_retrofit_nonce' );
        ...
        wp_safe_redirect( admin_url( 'admin.php?page=sab-settings&retrofitted=' . $updated_count ) );
        exit;
    }
}
