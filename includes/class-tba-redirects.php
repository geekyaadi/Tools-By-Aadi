<?php
/**
 * Redirect Manager & 404 Error Log Tracker Engine for Tools By Aadi
...
class TBA_Redirects {
    ...
    public static function init() {
        ...
        add_action( 'admin_post_tba_save_redirect_rule', [ __CLASS__, 'handle_save_redirect_rule' ] );
        add_action( 'admin_post_tba_delete_redirect_rule', [ __CLASS__, 'handle_delete_redirect_rule' ] );
        add_action( 'admin_post_tba_convert_404_to_301', [ __CLASS__, 'handle_convert_404_to_301' ] );
        add_action( 'admin_post_tba_clear_404_logs', [ __CLASS__, 'handle_clear_404_logs' ] );
        add_action( 'admin_post_tba_save_404_redirect_settings', [ __CLASS__, 'handle_save_404_redirect_settings' ] );
    }
    ...
    public static function create_tables() {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
        $table_redirects = $wpdb->prefix . 'tba_redirects';
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
        $table_404s      = $wpdb->prefix . 'tba_404_logs';
    ...
    public static function handle_frontend_redirects_and_404s() {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
        $table_redir = $wpdb->prefix . 'tba_redirects';

        // 1. Check for Active Redirect Rules
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
        $rule = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}tba_redirects WHERE source_url = %s AND status = 'active' LIMIT 1", $path_clean ) );

        if ( $rule ) {
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
            $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}tba_redirects SET hit_count = hit_count + 1 WHERE id = %d", $rule->id ) );
        ...
        if ( is_404() ) {
            ...
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
            $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}tba_404_logs (url, is_image, hit_count, user_agent, last_detected) VALUES (%s, %d, 1, %s, %s) ON DUPLICATE KEY UPDATE hit_count = hit_count + 1, last_detected = %s", $path_clean, $is_image, $user_agent, current_time( 'mysql' ), current_time( 'mysql' ) ) );

            // Auto-redirect 404 pages to homepage if toggle switch is enabled
            if ( get_option( 'tba_redirect_404_to_home', '0' ) === '1' ) {
    ...
    public static function handle_save_404_redirect_settings() {
        ...
        check_admin_referer( 'tba_redirect_nonce' );

        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $enable = isset( $_POST['tba_redirect_404_to_home'] ) ? '1' : '0';
        update_option( 'tba_redirect_404_to_home', $enable );

        wp_safe_redirect( admin_url( 'admin.php?page=tba-redirects&settings_saved=true' ) );
        exit;
    }
    ...
    public static function handle_save_redirect_rule() {
        ...
        if ( ! empty( $source ) && ! empty( $target ) ) {
            ...
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
            $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}tba_redirects (source_url, target_url, redirect_type, status) VALUES (%s, %s, %d, 'active') ON DUPLICATE KEY UPDATE target_url = %s, redirect_type = %d, status = 'active'", $source_clean, $target, $type, $target, $type ) );
        }

        wp_safe_redirect( admin_url( 'admin.php?page=tba-redirects&updated=true' ) );
        exit;
    }
    ...
    public static function handle_delete_redirect_rule() {
        ...
        wp_safe_redirect( admin_url( 'admin.php?page=tba-redirects&deleted=true' ) );
        exit;
    }
    ...
    public static function handle_convert_404_to_301() {
        ...
        if ( ! empty( $source ) && ! empty( $target ) ) {
            ...
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
            $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}tba_redirects (source_url, target_url, redirect_type, status) VALUES (%s, %s, 301, 'active') ON DUPLICATE KEY UPDATE target_url = %s, status = 'active'", $source_clean, $target, $target ) );
        ...
        wp_safe_redirect( admin_url( 'admin.php?page=tba-redirects&converted=true' ) );
        exit;
    }
    ...
    public static function handle_clear_404_logs() {
        ...
        wp_safe_redirect( admin_url( 'admin.php?page=tba-redirects&logs_cleared=true' ) );
        exit;
    }
    ...
    public static function get_redirect_rules() {
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
        $table = $wpdb->prefix . 'tba_redirects';
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
    ...
    public static function get_404_logs( $filter = 'all' ) {
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
        $table = $wpdb->prefix . 'tba_404_logs';
