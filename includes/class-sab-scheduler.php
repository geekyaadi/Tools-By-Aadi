...
class SAB_Scheduler {
    ...
    const CRON_HOOK        = 'sab_scheduled_publish';
    const OPTION_ENABLED   = 'sab_schedule_enabled';
    const OPTION_PER_DAY   = 'sab_posts_per_day';
    const OPTION_NICHES    = 'sab_schedule_niches';
    ...
    public static function enable() {
        if ( function_exists( 'as_next_scheduled_action' ) && function_exists( 'as_schedule_recurring_action' ) ) {
            if ( ! as_next_scheduled_action( self::CRON_HOOK ) ) {
                as_schedule_recurring_action( time(), 3600, self::CRON_HOOK, [], 'soniji-auto-blogging' );
            }
        }
    ...
    public static function disable() {
        if ( function_exists( 'as_unschedule_all_actions' ) ) {
            as_unschedule_all_actions( self::CRON_HOOK, [], 'soniji-auto-blogging' );
        }
    ...
    public static function run_scheduled_generation() {
        ...
        for ( $i = 0; $i < $slots_to_run; $i++ ) {
            ...
            $queued = SAB_Queue::get_next_queued();
    ...
    private static function publish_queued_post( $queued ) {
        SAB_Queue::mark_processing( $queued->id );
        ...
        if ( ! empty( $queued->post_id ) ) {
            ...
            if ( is_wp_error( $result ) ) {
                SAB_Queue::mark_failed( $queued->id, $result->get_error_message() );
            } else {
                SAB_Queue::mark_published( $queued->id, $queued->post_id );
            }
            return;
        }
        ...
        if ( is_wp_error( $post_id ) ) {
            SAB_Queue::mark_failed( $queued->id, $post_id->get_error_message() );
        } else {
            SAB_Queue::mark_published( $queued->id, $post_id );
        }
    }
    ...
    public static function generate_and_publish(
        ...
        if ( $preferred_title ) {
            ...
            $titles_result = SAB_Gemini::get_title_suggestions( $session_id, $niche, $focus_keywords, $language );
        ...
        // Duplicate check
        if ( SAB_Duplicate_Check::is_duplicate( $title ) ) {
            return new WP_Error( 'duplicate', "Duplicate title skipped: {$title}" );
        }

        // Step 2: Article
        $article_result = SAB_Gemini::generate_article( $session_id, $title, $focus_keywords, $language );
        if ( is_wp_error( $article_result ) ) return $article_result;
        $article_content = $article_result['article'];

        // Step 3: Tags
        $tags_result = SAB_Gemini::generate_tags( $session_id, $title, $language, $tag_count );
        $tags        = is_wp_error( $tags_result ) ? [] : $tags_result['tags'];

        // Step 4: Meta
        $meta_result = SAB_Gemini::generate_meta_description( $session_id, $title, $focus_keywords, $language );
        $meta        = is_wp_error( $meta_result ) ? '' : $meta_result['meta'];

        // Step 5: Category
        if ( empty( $category ) ) {
            $cat_result = SAB_Gemini::suggest_category( $session_id, $title, $language );
            $category   = is_wp_error( $cat_result ) ? '' : $cat_result['category'];
        }

        // Step 6: Thumbnail
        $thumb_result = SAB_Gemini::generate_thumbnail( $session_id, $title, [], $t2i_opts );
        $thumb_data   = is_wp_error( $thumb_result ) ? null : $thumb_result['image_data'];

        // Step 7: OG Image
        $og_result  = SAB_Gemini::generate_og_image( $session_id, $title, $t2i_opts );
        $og_data    = is_wp_error( $og_result ) ? null : $og_result['image_data'];

        // Step 8: Alt Text
        $alt_result = SAB_Gemini::generate_alt_text( $session_id, $title, $language );
        ...
        if ( $silo_role === 'cluster' && ! empty( $silo_id ) ) {
            global $wpdb;
            $queue_table = SAB_Queue::table_name();
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
            $pillar_item = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$queue_table} WHERE id = %d AND status = 'published' AND post_id IS NOT NULL LIMIT 1", $silo_id ) );

            if ( $pillar_item ) {
                $pillar_url = get_permalink( $pillar_item->post_id );
                if ( $pillar_url ) {
                    $backlink_html = "\n\n<p class=\"sab-silo-backlink\">Learn more about this topic in our comprehensive guide: <a href=\"" . esc_url( $pillar_url ) . "\">" . esc_html( $pillar_item->title ) . "</a>.</p>";
        ...
        if ( $silo_role === 'cluster' && ! empty( $silo_id ) && ! is_wp_error( $post_id ) ) {
            ...
            if ( $pillar_item ) {
                ...
                if ( $pillar_post ) {
                    ...
                    if ( strpos( $pillar_content, 'class="sab-silo-links"' ) !== false ) {
                        $pos = strrpos( $pillar_content, '</ul>' );
                        if ( $pos !== false ) {
                            $pillar_content = substr_replace( $pillar_content, $new_link_html . '</ul>', $pos, 5 );
                        }
                    } else {
                        $pillar_content .= "\n\n<h3>Related Articles</h3><ul class=\"sab-silo-links\">" . $new_link_html . "</ul>";
                    }
    ...
    private static function pick_next_niche( array $niches ) {
        $last_used = get_option( 'sab_last_niche_index', -1 );
    ...
    private static function record_niche_used( string $niche ) {
        ...
        if ( $index !== false ) {
            update_option( 'sab_last_niche_index', $index );
        }
    }
    ...
    public static function get_next_run() {
        ...
        return $ts ? date_i18n( 'Y-m-d H:i:s', $ts ) : __( 'Not scheduled', 'soniji-auto-blogging' );
    }
}
