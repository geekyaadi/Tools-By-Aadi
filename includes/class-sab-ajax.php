...
class SAB_Ajax {
    ...
    public static function init() {
        $actions = [
            ...
            'sab_check_duplicate',
            'sab_get_step_status',
            'sab_save_reference_image',
            'sab_delete_reference_image',
            'sab_ping_key',
            'sab_ping_all_keys',
            'sab_generate_planner_titles',
            'sab_save_planner_tasks',
            'sab_process_queue_item',
            'sab_generate_pending_thumbnail',
            'sab_generate_tags',
            'sab_translate_post',
            'sab_request_indexing',
            'sab_rewrite_post',
        ];
    ...
    private static function verify_nonce() {
        if ( ! check_ajax_referer( 'sab_nonce', 'nonce', false ) ) {
            self::error( __( 'Security check failed.', 'soniji-auto-blogging' ) );
            exit;
        }
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( __( 'Unauthorized capability check failed.', 'soniji-auto-blogging' ) );
            exit;
        }
    }
    ...
    public static function handle_sab_get_titles() {
        self::verify_nonce();
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $session_id     = sanitize_text_field( wp_unslash( $_POST['session_id'] ?? uniqid( 'sab_'  ) ) );

        if ( empty( $niche ) ) {
            self::error( __( 'Please enter a niche.', 'soniji-auto-blogging' ) );
            return;
        }

        $result = SAB_Gemini::get_title_suggestions( $session_id, $niche, $focus_keywords );
        ...
        self::success( [
            ...
            'key_used'   => SAB_Key_Manager::mask_key( $result['key_used'] ?? '' ),
    ...
    public static function handle_sab_generate_post() {
        self::verify_nonce();
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $post_status    = sanitize_text_field( wp_unslash( $_POST['post_status'] ?? get_option( 'sab_default_status', 'draft'  ) ) );
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $skip_publish   = (bool) ( $_POST['preview_only'] ?? false );

        if ( empty( $session_id ) || empty( $title ) ) {
            self::error( __( 'Missing session or title.', 'soniji-auto-blogging' ) );
            return;
        }

        // Review Mode: always force draft
        if ( get_option( 'sab_review_mode', 0 ) ) {
            $post_status = 'draft';
        }
        ...
        switch ( $step ) {
            ...
            // ------------------------------------------------------------------
            case 'article':
                $result = SAB_Gemini::generate_article( $session_id, $title, $focus_keywords );
            ...
            case 'tags':
                ...
                $result = SAB_Gemini::generate_tags( $session_id, $title, 'English', $tag_count );
            ...
            // ------------------------------------------------------------------
            case 'meta':
                $result = SAB_Gemini::generate_meta_description( $session_id, $title, $focus_keywords );
            ...
            case 'category':
                ...
                $result = SAB_Gemini::suggest_category( $session_id, $title );
            ...
            case 'thumbnail':
                ...
                $result = SAB_Gemini::generate_thumbnail( $session_id, $title, $ref_image, $t2i_opts );
                ...
                self::success( [
                    ...
                    'cached'         => $result['cached'],
                    'key_used'       => $result['key_used'] ? SAB_Key_Manager::mask_key( $result['key_used'] ) : '',
                    'switched'       => $result['switched'] ?? false,
                ] );
                break;
            ...
            case 'og_image':
                ...
                $result = SAB_Gemini::generate_og_image( $session_id, $title, $t2i_opts );
            ...
            // ------------------------------------------------------------------
            case 'alt_text':
                $result = SAB_Gemini::generate_alt_text( $session_id, $title );
            ...
            case 'publish':
                ...
                $article      = SAB_Gemini::get_cached_step( $session_id, 'article' );
                $tags         = SAB_Gemini::get_cached_step( $session_id, 'tags' );
                $meta         = SAB_Gemini::get_cached_step( $session_id, 'meta' );
                $category     = SAB_Gemini::get_cached_step( $session_id, 'cat' );
                $thumb_data   = SAB_Gemini::get_cached_step( $session_id, 'thumb' );
                $og_data      = SAB_Gemini::get_cached_step( $session_id, 'og' );
                $alt_text     = SAB_Gemini::get_cached_step( $session_id, 'alt' );

                if ( ! $article ) {
                    self::error( __( 'Article data not found. Please regenerate.', 'soniji-auto-blogging' ) );
                    return;
                }
                ...
                // Duplicate check
                $dup = SAB_Duplicate_Check::find_duplicate( $title );
                if ( $dup ) {
                    self::success( [
                ...
                $post_id = SAB_Post_Creator::create( [
                    'title'            => $title,
                    'article'          => $article,
                    'tags'             => $tags ?: [],
                ...
                // Log to history
                $token_est = SAB_History::estimate_tokens(
                    $article,
                    is_array( $tags ) ? implode( ',', $tags ) : '',
                    $meta ?: ''
                );
                $history_id = SAB_History::insert( [
                    'session_id'     => $session_id,
                    'niche'          => $niche,
                    'title'          => $title,
                    'post_id'        => $post_id,
                    'status'         => 'success',
                    'key_used'       => SAB_Gemini::get_current_key( $session_id ) ?? '',
                    'token_estimate' => $token_est,
                ] );

                SAB_Gemini::clear_session_cache( $session_id );
            ...
            default:
                self::error( __( 'Unknown generation step.', 'soniji-auto-blogging' ) );
        }
    }
    ...
    public static function handle_sab_check_duplicate() {
        self::verify_nonce();
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $title = sanitize_text_field( wp_unslash( $_POST['title'] ?? ''  ) );
        $dup   = SAB_Duplicate_Check::find_duplicate( $title );
    ...
    public static function handle_sab_get_step_status() {
        self::verify_nonce();
        // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        $session_id = sanitize_text_field( wp_unslash( $_POST['session_id'] ?? ''  ) );
        ...
        $done  = [];
        foreach ( $steps as $s ) {
            if ( SAB_Gemini::get_cached_step( $session_id, $s ) ) {
                $done[] = $s;
            }
        }
        ...
        self::success( [
            ...
            'current_key'     => SAB_Key_Manager::mask_key( SAB_Gemini::get_current_key( $session_id ) ?? '' ),
        ] );
    }
    ...
    public static function handle_sab_save_reference_image() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
        ...
        SAB_Gemini::save_default_reference_image( $base64, $mime_type );
    ...
    public static function handle_sab_delete_reference_image() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        SAB_Gemini::delete_default_reference_image();
    ...
    public static function handle_sab_ping_key() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $result  = SAB_Key_Manager::ping_key( $api_key );
        ...
        SAB_Key_Manager::save_all_keys( $keys );
        ...
        self::success( [
            ...
            'reset_in_human' => $reset_in_secs !== null
                                    ? SAB_Key_Manager::format_seconds( $reset_in_secs )
                                    : null,
        ] );
    }
    ...
    public static function handle_sab_ping_all_keys() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $raw_results = SAB_Key_Manager::ping_all_keys();
        $keys        = SAB_Key_Manager::get_all_keys();
        ...
        $stats = SAB_Key_Manager::get_stats();
        self::success( [
            'results' => $formatted,
            'summary' => [
                'active'    => $stats['active'],
    ...
    public static function handle_sab_generate_planner_titles() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        if ( $mode === 'silo' ) {
            $result = SAB_Gemini::get_silo_suggestions( $session_id, $niche, $language );
            if ( is_wp_error( $result ) ) {
                self::error( $result->get_error_message() );
                return;
            }
        ...
        $result = SAB_Gemini::get_planner_title_suggestions( $session_id, $niche, $language, $count );
        ...
        if ( empty( $titles ) ) {
            self::error( __( 'No titles could be parsed. Try again.', 'soniji-auto-blogging' ) );
            return;
        }
    ...
    public static function handle_sab_save_planner_tasks() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        if ( empty( $niche ) || empty( $tasks ) ) {
            self::error( __( 'Missing niche or task list.', 'soniji-auto-blogging' ) );
            return;
        }
        ...
        if ( $mode === 'silo' ) {
            ...
            if ( $pillar_title ) {
                $pillar_queue_id = SAB_Queue::enqueue( $niche, $pillar_title, $language, $pillar_cat, 0, null, 'pillar', $tag_count, $meta );
                if ( $pillar_queue_id ) {
                    // Update it to have silo_id = itself
                    global $wpdb;
                    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                    $wpdb->update( SAB_Queue::table_name(), [ 'silo_id' => $pillar_queue_id ], [ 'id' => $pillar_queue_id ] );
                    $count++;
                    ...
                    foreach ( $tasks as $task ) {
                        ...
                        if ( $title ) {
                            SAB_Queue::enqueue( $niche, $title, $language, $category, 0, $pillar_queue_id, 'cluster', $tag_count, $meta );
                            $count++;
                        }
                    }
                }
            ...
            foreach ( $tasks as $task ) {
                ...
                if ( $title ) {
                    SAB_Queue::enqueue( $niche, $title, $language, $category, 0, null, null, $tag_count, $meta );
                    $count++;
                }
            }
        }
        ...
        self::success( [
            ...
            'message' => sprintf( __( 'Successfully enqueued %d tasks in the background worker.', 'soniji-auto-blogging' ), $count )
        ] );
    }
    ...
    public static function handle_sab_process_queue_item() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $stats = SAB_Key_Manager::get_stats();
        if ( $stats['active'] === 0 ) {
            self::error( __( 'All API keys are exhausted. Pausing queue processor.', 'soniji-auto-blogging' ) );
            return;
        }

        $next = SAB_Queue::get_next_queued();
        if ( ! $next ) {
            self::success( [ 'processed' => false, 'message' => __( 'Queue is empty.', 'soniji-auto-blogging' ) ] );
            return;
        }

        // Process item
        SAB_Queue::mark_processing( $next->id );
        ...
        if ( ! empty( $next->post_id ) ) {
            ...
            if ( is_wp_error( $res ) ) {
                SAB_Queue::mark_failed( $next->id, $res->get_error_message() );
                self::error( $res->get_error_message() );
                return;
            }
            SAB_Queue::mark_published( $next->id, $next->post_id );
        ...
        $post_id = SAB_Scheduler::generate_and_publish(
            $next->niche,
            $next->title,
            '',
        ...
        if ( is_wp_error( $post_id ) ) {
            SAB_Queue::mark_failed( $next->id, $post_id->get_error_message() );
            self::error( $post_id->get_error_message(), [ 'id' => $next->id ] );
            return;
        }

        SAB_Queue::mark_published( $next->id, $post_id );
        self::success( [
            'processed' => true,
            'id'        => $next->id,
            'title'     => $next->title,
    ...
    public static function handle_sab_generate_pending_thumbnail() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        if ( ! in_array( $engine, [ 'ai', 'text_to_image' ], true ) ) {
            $engine = get_option( 'sab_thumb_type', 'ai' );
        }

        $session_id = 'thumb_regen_' . uniqid();
        $result = SAB_Gemini::generate_thumbnail( $session_id, $post->post_title, [], [ 'thumb_type' => $engine ] );
        ...
        $attachment_id = SAB_Post_Creator::upload_image(
            $image_data['base64'],
        ...
        delete_post_meta( $post_id, '_sab_thumbnail_pending' );
    ...
    public static function handle_sab_generate_tags() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $result = SAB_Gemini::generate_tags( $session_id, $post->post_title, 'English', $tag_count );
    ...
    public static function handle_sab_translate_post() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        // 1. Translate Title
        $title_res = SAB_Gemini::translate_text( $session_id, $post->post_title, $target_lang );
        ...
        $article_res = SAB_Gemini::translate_html( $session_id, $post->post_content, $target_lang );
        ...
        if ( empty( $orig_meta ) ) {
            $orig_meta = get_post_meta( $post_id, '_sab_meta_description', true );
        }
        
        $translated_meta = '';
        if ( ! empty( $orig_meta ) ) {
            $meta_res = SAB_Gemini::translate_text( $session_id, $orig_meta, $target_lang );
        ...
        if ( ! empty( $orig_tags ) ) {
            ...
            $tags_res = SAB_Gemini::translate_text( $session_id, $tags_string, $target_lang );
        ...
        // Meta flags
        update_post_meta( $new_post_id, '_sab_generated', '1' );
        update_post_meta( $new_post_id, '_sab_translated_from', $post_id );
        update_post_meta( $new_post_id, '_sab_translation_lang', $target_lang );
        ...
        if ( ! empty( $translated_meta ) ) {
            update_post_meta( $new_post_id, '_sab_meta_description', $translated_meta );
    ...
    public static function handle_sab_request_indexing() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
        ...
        $url    = get_permalink( $post_id );
        $result = SAB_GSC_Helper::submit_url( $url, 'URL_UPDATED' );
        ...
        update_post_meta( $post_id, '_sab_gsc_last_ping', current_time( 'mysql' ) );
    ...
    public static function handle_sab_rewrite_post() {
        self::verify_nonce();
        if ( ! current_user_can( 'manage_options' ) ) {
            self::error( 'Unauthorized' );
            return;
...
\"\"\"";
        ...
        $result = SAB_Gemini::request( $session_id, SAB_Gemini::get_text_model(), [
            'contents' => [
                [ 'parts' => [ [ 'text' => $prompt ] ] ]
            ],
            'generationConfig' => [
        ...
        $rewritten = SAB_Gemini::extract_text( $result['data'] );
