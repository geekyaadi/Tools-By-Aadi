<?php
/**
 * XML Sitemap Manager & Controller Engine for Tools By Aadi
...
class TBA_Sitemap {
    ...
    public static function add_rewrite_rules() {
        ...
        add_rewrite_rule( '^' . preg_quote( $slug_clean, '#' ) . '$', 'index.php?tba_sitemap=index', 'top' );
        add_rewrite_rule( '^sitemap_index\.xml$', 'index.php?tba_sitemap=index', 'top' );
        add_rewrite_rule( '^sitemap\.xml$', 'index.php?tba_sitemap=index', 'top' );
        add_rewrite_rule( '^post-sitemap\.xml$', 'index.php?tba_sitemap=posts', 'top' );
        add_rewrite_rule( '^page-sitemap\.xml$', 'index.php?tba_sitemap=pages', 'top' );
        add_rewrite_rule( '^category-sitemap\.xml$', 'index.php?tba_sitemap=categories', 'top' );
        add_rewrite_rule( '^tag-sitemap\.xml$', 'index.php?tba_sitemap=tags', 'top' );
    }
    ...
    public static function check_request_sitemap( $wp ) {
        ...
        if ( isset( $wp->query_vars['tba_sitemap'] ) || isset( $_GET['tba_sitemap'] ) ) {
            self::render_sitemap();
        }
    ...
    public static function render_sitemap() {
        ...
        $type = get_query_var( 'tba_sitemap' );
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        if ( empty( $type ) && isset( $_GET['tba_sitemap'] ) ) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
            $type = sanitize_text_field( $_GET['tba_sitemap'] );
        }
        ...
        $mode = get_option( 'tba_sitemap_mode', 'index' ); // 'index' or 'single'
    ...
    private static function render_index_sitemap() {
        ...
        echo '<?xml-stylesheet type="text/xsl" href="' . esc_url( TBA_PLUGIN_URL . 'admin/sitemap-style.xsl' ) . '"?>' . "\n";
        echo '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">' . "\n";

        if ( get_option( 'tba_sitemap_enable_posts', '1' ) === '1' ) {
            echo '  <sitemap>' . "\n";
        ...
        if ( get_option( 'tba_sitemap_enable_pages', '1' ) === '1' ) {
            echo '  <sitemap>' . "\n";
        ...
        if ( get_option( 'tba_sitemap_enable_cats', '1' ) === '1' ) {
            echo '  <sitemap>' . "\n";
        ...
        if ( get_option( 'tba_sitemap_enable_tags', '1' ) === '1' ) {
            echo '  <sitemap>' . "\n";
    ...
    private static function render_posts_sitemap() {
        ...
        $include_imgs  = get_option( 'tba_sitemap_include_images', '1' ) === '1';
    ...
    private static function render_combined_sitemap() {
        $enable_home   = get_option( 'tba_sitemap_enable_home', '1' ) === '1';
        $home_priority = get_option( 'tba_sitemap_priority_home', '1.0' );
        $home_freq     = get_option( 'tba_sitemap_changefreq_home', 'daily' );

        $enable_posts  = get_option( 'tba_sitemap_enable_posts', '1' ) === '1';
        $post_priority = get_option( 'tba_sitemap_priority_post', '0.8' );
        $post_freq     = get_option( 'tba_sitemap_changefreq_post', 'daily' );

        $enable_pages  = get_option( 'tba_sitemap_enable_pages', '1' ) === '1';
        $page_priority = get_option( 'tba_sitemap_priority_page', '0.6' );
        $page_freq     = get_option( 'tba_sitemap_changefreq_page', 'weekly' );

        $enable_cats   = get_option( 'tba_sitemap_enable_cats', '1' ) === '1';
        $cat_priority  = get_option( 'tba_sitemap_priority_cat', '0.5' );
        $cat_freq      = get_option( 'tba_sitemap_changefreq_cat', 'weekly' );

        $enable_tags   = get_option( 'tba_sitemap_enable_tags', '1' ) === '1';
        $tag_priority  = get_option( 'tba_sitemap_priority_tag', '0.4' );
        $tag_freq      = get_option( 'tba_sitemap_changefreq_tag', 'monthly' );
    ...
    public static function ping_search_engines( $post_id ) {
        ...
        $sitemap_url = home_url( '/' . get_option( 'tba_sitemap_slug', 'sitemap.xml' ) );

        if ( get_option( 'tba_sitemap_auto_ping_google', '1' ) === '1' ) {
            wp_remote_get( 'https://www.google.com/ping?sitemap=' . urlencode( $sitemap_url ), [ 'timeout' => 5 ] );
        }
        if ( get_option( 'tba_sitemap_auto_ping_bing', '1' ) === '1' ) {
