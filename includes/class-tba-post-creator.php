...
class TBA_Post_Creator {
    ...
    public static function create( array $data ) {
        $post_status = $data['post_status'] ?? get_option( 'tba_default_status', 'draft' );
        $author_id   = $data['author_id']   ?? (int) get_option( 'tba_default_author', get_current_user_id() );
        ...
        if ( get_option( 'tba_enable_toc', 1 ) ) {
        ...
        if ( get_option( 'tba_enable_faq', 1 ) ) {
            $session_id = 'faq_' . uniqid();
            $language   = $data['language'] ?? 'English';
            $faq_count  = (int) get_option( 'tba_faq_count', 3 );
            $faq_res    = TBA_Gemini::generate_faq( $session_id, $data['title'], $language, $faq_count );

            if ( ! is_wp_error( $faq_res ) && ! empty( $faq_res['faqs'] ) ) {
                $faqs = $faq_res['faqs'];
                $faq_html = "\n\n<div class=\"tba-faq-wrapper\" style=\"margin-top: 30px; border-top: 1px solid #e2e8f0; padding-top: 20px;\">";
                $faq_html .= "<h3 class=\"tba-faq-heading\" style=\"margin-bottom: 15px; font-weight: 700; color: #1e293b;\">💡 Frequently Asked Questions</h3>";
                $faq_html .= "<div class=\"tba-faq-list\">";
                ...
                foreach ( $faqs as $faq ) {
                    ...
                    $faq_html .= "<details class=\"tba-faq-item\" style=\"border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 10px; padding: 12px 16px; background: #f8fafc;\">";
                    $faq_html .= "<summary class=\"tba-faq-question\" style=\"font-weight: 600; color: #0f172a; cursor: pointer; outline: none;\">" . $q . "</summary>";
                    $faq_html .= "<div class=\"tba-faq-answer\" style=\"margin-top: 8px; color: #475569; line-height: 1.6;\">" . $a . "</div>";
                    $faq_html .= "</details>";
        ...
        if ( get_option( 'tba_speed_lazy_loading', '1' ) === '1' ) {
        ...
        if ( get_option( 'tba_speed_html_minification', '1' ) === '1' ) {
        ...
        if ( get_option( 'tba_enable_internal_linking', 1 ) ) {
            $current_content = self::apply_internal_linking( $post_id, $current_content );
        }

        if ( class_exists( 'TBA_Outbound_Linker' ) ) {
            $current_content = TBA_Outbound_Linker::process( $current_content, $data['title'] );
        }
        ...
        if ( get_option( 'tba_speed_auto_cache_purge', '1' ) === '1' && function_exists( 'tba_purge_all_caches' ) ) {
        ...
        if ( ! empty( $data['og_image_data'] ) ) {
            ...
            if ( ! is_wp_error( $og_id ) ) {
                update_post_meta( $post_id, '_tba_og_image_id', $og_id );
        ...
        update_post_meta( $post_id, '_tba_generated', true );
        update_post_meta( $post_id, '_tba_generated_at', current_time( 'mysql' ) );
    ...
    public static function inject_table_of_contents( string $content ) {
        ...
        $toc_html = "\n<div class=\"tba-toc-container\" style=\"background:#0f172a; border:1px solid #1e293b; border-radius:10px; padding:18px 22px; margin:25px 0; font-family:system-ui,-apple-system,sans-serif;\">";
        $toc_html .= "<div class=\"tba-toc-header\" style=\"display:flex; justify-content:space-between; align-items:center;\">";
        $toc_html .= "<strong style=\"color:#f8fafc; font-size:16px; font-weight:700;\">📌 Table of Contents</strong>";
        $toc_html .= "</div>";
        $toc_html .= "<ol class=\"tba-toc-list\" style=\"margin-top:12px; margin-bottom:0; padding-left:22px; color:#94a3b8; font-size:14px; line-height:1.8;\">";
    ...
    private static function set_meta_description( int $post_id, string $meta, string $title = '', string $focus_keywords = '' ) {
        ...
        // Generic fallback
        update_post_meta( $post_id, '_tba_meta_description', $meta );
    }
    ...
    public static function upload_image( string $base64, string $mime_type, string $filename_base, string $alt_text = '' ) {
        ...
        if ( get_option( 'tba_enable_text_overlay', 0 ) && strpos( $filename_base, '-thumbnail' ) !== false ) {
    ...
    public static function apply_internal_linking( int $post_id, string $content ) {
        ...
        $max_links  = (int) get_option( 'tba_max_internal_links', 3 );
        $link_style = get_option( 'tba_internal_link_style', 'callout_box' );
        if ( $max_links <= 0 ) {
            return $content;
        }
                ...
                $content = preg_replace_callback( $pattern, function( $matches ) use ( &$link_count, &$replaced, $max_links, $url ) {
                    if ( isset( $matches[1] ) && $matches[1] !== '' ) {
                        if ( $link_count < $max_links && ! $replaced ) {
                            $link_count++;
                            $replaced = true;
                            return '<a href="' . esc_url( $url ) . '" class="tba-internal-link" style="color: #0284c7; text-decoration: underline; font-weight: 600;">' . esc_html( $matches[1] ) . '</a>';
                        }
                    }
                    return $matches[0];
                }, $content );
    ...
    private static function format_link_html( string $title, string $url, string $link_style ): string {
        if ( $link_style === 'card' ) {
            $html = "\n<div class=\"tba-related-card\" style=\"background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); border: 1px solid #334155; border-radius: 10px; padding: 16px 20px; margin: 24px 0; font-family: system-ui, -apple-system, sans-serif;\">";
            ...
            $html = "\n<div class=\"tba-related-box\" style=\"background: #f8fafc; border-left: 4px solid #0284c7; padding: 12px 18px; margin: 20px 0; border-radius: 6px; font-family: system-ui, -apple-system, sans-serif;\">";
    ...
    private static function generate_auto_comments( int $post_id, string $title ) {
        if ( ! get_option( 'tba_enable_comments', 0 ) ) {
            return;
        }

        $count = (int) get_option( 'tba_comments_count', 2 );
        if ( $count <= 0 ) {
            return;
        }

        $session_id = 'comments_' . uniqid();
        $result = TBA_Gemini::generate_comments( $session_id, $title, $count );
    ...
    private static function draw_text_overlay( $image, string $text ) {
        ...
        $font_size = (int) get_option( 'tba_overlay_font_size', 24 );
        $text_color_hex = get_option( 'tba_overlay_color', '#ffffff' );
        $bg_color_hex   = get_option( 'tba_overlay_bg_color', '#000000' );
        $bg_opacity     = (int) get_option( 'tba_overlay_bg_opacity', 60 );
        $position       = get_option( 'tba_overlay_position', 'bottom' );

        $font_path = TBA_PLUGIN_DIR . 'admin/fonts/Roboto-Bold.ttf';
