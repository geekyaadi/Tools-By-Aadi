...
$site_name = get_option( 'sab_site_name', get_bloginfo( 'name' ) );
$site_url  = get_option( 'sab_site_url', home_url( '/' ) );
$email     = get_option( 'sab_contact_email', get_bloginfo( 'admin_email' ) );

$cookie_enable        = get_option( 'sab_cookie_enable', '1' );
$cookie_style         = get_option( 'sab_cookie_style', 'bottom_banner' );
$cookie_text          = get_option( 'sab_cookie_text', 'We use cookies to personalize content, ads, and analyze traffic for maximum user experience.' );
$cookie_btn_text      = get_option( 'sab_cookie_btn_text', 'Accept All Cookies 🍪' );
$cookie_enable_reject = get_option( 'sab_cookie_enable_reject', '0' );
$cookie_reject_text   = get_option( 'sab_cookie_reject_btn_text', 'Decline Non-Essential' );
$cookie_privacy_url   = get_option( 'sab_cookie_privacy_url', home_url('/privacy-policy/') );
...
<div class="sab-wrap">
    ...
    <div class="sab-header">
        <div class="sab-header-inner">
            <div class="sab-logo">
                ...
                <span class="sab-logo-badge">📄 Pages & Cookie Consent</span>
            </div>
    ...
    <a href="<?php echo esc_url( admin_url('admin.php?page=sab-pages') ); ?>" class="sab-nav-link active">Pages Generator</a>
    ...
    <div class="sab-content">
        ...
        <div class="sab-panel" style="margin-bottom:25px;">
            <div class="sab-panel-header" style="padding-bottom:12px; border-bottom:1px solid #e2e8f0; margin-bottom:18px;">
                <h2 class="sab-panel-title" style="color:var(--sab-text-dark); margin:0 0 4px 0; font-size:16px; font-weight:600;">📄 One-Click Essential &amp; Legal Pages Generator</h2>
                <p style="color:var(--sab-text-muted); margin:0; font-size:13px;">Instantly generate 1000+ words, GDPR/AdSense compliant human-written pages tailored for your website.</p>
            </div>
            <div class="sab-panel-body">
                <form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
                    <?php wp_nonce_field('sab_generate_essential_pages'); ?>
                    <input type="hidden" name="action" value="sab_generate_essential_pages">

                    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap:20px; margin-bottom:25px;">
                        <div>
                            <label style="font-weight:600; display:block; margin-bottom:6px;">Website / Business Name</label>
                            <input type="text" name="sab_site_name" value="<?php echo esc_attr($site_name); ?>" class="regular-text" style="width:100%; font-size:14px;" required placeholder="My Awesome Website">
                        </div>
                        <div>
                            <label style="font-weight:600; display:block; margin-bottom:6px;">Website Full URL</label>
                            <input type="url" name="sab_site_url" value="<?php echo esc_url($site_url); ?>" class="regular-text" style="width:100%; font-size:14px;" required placeholder="https://mywebsite.com">
                        </div>
                        <div>
                            <label style="font-weight:600; display:block; margin-bottom:6px;">Official Contact Email</label>
                            <input type="email" name="sab_contact_email" value="<?php echo esc_attr($email); ?>" class="regular-text" style="width:100%; font-size:14px;" required placeholder="contact@mywebsite.com">
                        </div>
                    </div>
        ...
        <div class="sab-panel">
            <div class="sab-panel-header" style="padding-bottom:12px; border-bottom:1px solid #e2e8f0; margin-bottom:18px;">
                <h2 class="sab-panel-title" style="color:var(--sab-text-dark); margin:0 0 4px 0; font-size:16px; font-weight:600;">🍪 Cookie Consent Banner Controller</h2>
                <p style="color:var(--sab-text-muted); margin:0; font-size:13px;">Customize style, layout, text content, and toggle cookie consent popup on your website.</p>
            </div>
            <div class="sab-panel-body" style="padding:24px;">
                <form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
                    <?php wp_nonce_field('sab_save_cookie_settings'); ?>
                    <input type="hidden" name="action" value="sab_save_cookie_settings">

                    <table class="form-table sab-settings-table">
                        <tr>
                            <th scope="row"><label for="sab_cookie_enable">Enable Cookie Consent Banner</label></th>
                            <td>
                                <label class="sab-switch">
                                    <input type="checkbox" id="sab_cookie_enable" name="sab_cookie_enable" value="1" <?php checked($cookie_enable, '1'); ?>>
                        ...
                        <tr>
                            <th scope="row"><label for="sab_cookie_style">Banner Style & Layout</label></th>
                            <td>
                                <select id="sab_cookie_style" name="sab_cookie_style" class="regular-text">
                        ...
                        <tr>
                            <th scope="row"><label for="sab_cookie_text">Consent Message Text</label></th>
                            <td>
                                <textarea id="sab_cookie_text" name="sab_cookie_text" rows="3" class="large-text"><?php echo esc_textarea($cookie_text); ?></textarea>
                            </td>
                        </tr>

                        <tr>
                            <th scope="row"><label for="sab_cookie_btn_text">Accept Button Text</label></th>
                            <td>
                                <input type="text" id="sab_cookie_btn_text" name="sab_cookie_btn_text" value="<?php echo esc_attr($cookie_btn_text); ?>" class="regular-text">
                            </td>
                        </tr>

                        <tr>
                            <th scope="row"><label for="sab_cookie_enable_reject">Include Optional Reject / Decline Button</label></th>
                            <td>
                                <label class="sab-switch">
                                    <input type="checkbox" id="sab_cookie_enable_reject" name="sab_cookie_enable_reject" value="1" <?php checked($cookie_enable_reject, '1'); ?>>
                                    <span class="sab-slider round"></span>
                                </label>
                        ...
                        <tr>
                            <th scope="row"><label for="sab_cookie_reject_btn_text">Reject Button Text</label></th>
                            <td>
                                <input type="text" id="sab_cookie_reject_btn_text" name="sab_cookie_reject_btn_text" value="<?php echo esc_attr($cookie_reject_text); ?>" class="regular-text" placeholder="Decline Non-Essential">
                            </td>
                        </tr>

                        <tr>
                            <th scope="row"><label for="sab_cookie_privacy_url">Privacy Policy Page Link</label></th>
                            <td>
                                <input type="url" id="sab_cookie_privacy_url" name="sab_cookie_privacy_url" value="<?php echo esc_url($cookie_privacy_url); ?>" class="regular-text">
                            </td>
                        </tr>
                    </table>

                    <div style="margin-top:20px;">
                        <button type="submit" class="sab-btn sab-btn-primary" style="padding:12px 25px; font-size:15px; background:#0f172a; border:none;">💾 Save Cookie Consent Settings</button>
                    </div>
                </form>
            </div>
        </div>
