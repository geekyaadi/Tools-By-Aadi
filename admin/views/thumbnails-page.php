...
<div class="sab-wrap">
    <div class="sab-header">
        <div class="sab-header-inner">
            <div class="sab-logo">
                ...
                <span class="sab-logo-badge">Thumbnail Manager</span>
            </div>
    ...
    <a href="<?php echo esc_url( admin_url('admin.php?page=sab-thumbnails') ); ?>" class="sab-nav-link active">Thumbnail Tool</a>
    ...
    <!-- Tab navigation -->
    <div class="sab-tabs" style="display:flex; gap:10px; margin-bottom:20px; border-bottom:1px solid rgba(255,255,255,0.06); padding-bottom:10px;">
        <a href="<?php echo esc_url( admin_url('admin.php?page=sab-thumbnails&tab=pending') ); ?>" class="sab-btn <?php echo $tab === 'pending' ? 'sab-btn-primary' : 'sab-btn-secondary'; ?>" style="font-weight:600; border-radius:6px; font-size:12px; padding:8px 16px;">
            ⚠️ Pending Thumbnails (<?php echo (int) $total_pending; ?>)
        </a>
        <a href="<?php echo esc_url( admin_url('admin.php?page=sab-thumbnails&tab=completed') ); ?>" class="sab-btn <?php echo $tab === 'completed' ? 'sab-btn-primary' : 'sab-btn-secondary'; ?>" style="font-weight:600; border-radius:6px; font-size:12px; padding:8px 16px;">
    ...
    <div class="sab-content">
        ...
        <div class="sab-panel">
            <div class="sab-panel-header">
                <h2 class="sab-panel-title">⚠️ Pending Thumbnails List</h2>
            </div>
            
            <?php if ( empty($display_posts) ): ?>
            <div class="sab-empty-state">🎉 All generated posts have thumbnails! No pending thumbnails found.</div>
            <?php else: ?>
            ...
            <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap; padding:15px 0 10px;">
                ...
                <select id="sab-bulk-thumb-engine" class="sab-select" style="width:auto; min-width:200px; font-size:12px; padding:6px 10px;">
                    <option value="ai">⚡ AI Generated (Gemini)</option>
                    <option value="text_to_image">🎨 Title Text Thumbnail (GD)</option>
                </select>
                <button type="button" id="sab-btn-generate-selected-thumbs" class="sab-btn sab-btn-primary sab-btn-small" style="background:#6366f1; border-color:#6366f1;" disabled>
                    🖼️ Generate Selected
                </button>
                <button type="button" id="sab-btn-generate-all-thumbs" class="sab-btn sab-btn-primary sab-btn-small" style="background:linear-gradient(135deg,#059669,#10b981); border:none;">
                    🚀 Generate All Pending (<?php echo (int) $total_pending; ?>)
                </button>
            </div>

            <!-- Bulk Progress Bar -->
            <div id="sab-bulk-thumb-progress" style="display:none; margin-bottom:12px;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:6px;">
                    <span id="sab-bulk-thumb-status" style="font-size:12px; color:#a5b4fc; font-weight:600;">Processing...</span>
                    <span id="sab-bulk-thumb-count" style="font-size:11px; color:#94a3b8;">0 / 0</span>
                </div>
                <div style="width:100%; height:8px; background:#1e293b; border-radius:4px; overflow:hidden;">
                    <div id="sab-bulk-thumb-bar" style="width:0%; height:100%; background:linear-gradient(90deg,#6366f1,#10b981); border-radius:4px; transition:width 0.4s ease;"></div>
                </div>
            </div>

            <table class="sab-table" id="sab-pending-thumbs-table">
                <thead>
                    <tr>
                        <th width="40"><input type="checkbox" id="sab-thumb-select-all" title="Select All"></th>
                ...
                <tbody>
                    ...
                    ?>
                    <tr id="sab-thumb-row-<?php echo (int) $p->ID; ?>">
                        <td><input type="checkbox" class="sab-thumb-checkbox" data-post-id="<?php echo (int) $p->ID; ?>"></td>
                        ...
                        <td>
                            <span class="sab-status-badge sab-status-exhausted">🖼️ Missing Thumbnail</span>
                        </td>
                        <td class="sab-thumb-action-cell" style="vertical-align: middle;">
                            <button type="button" class="sab-btn sab-btn-primary sab-btn-small sab-btn-gen-thumb-ai" data-post-id="<?php echo (int) $p->ID; ?>" style="margin-bottom: 5px; display: block; width: 100%; text-align: center;">
                                ⚡ Generate AI Thumbnail
                            </button>
                            <button type="button" class="sab-btn sab-btn-secondary sab-btn-small sab-btn-gen-thumb-t2i" data-post-id="<?php echo (int) $p->ID; ?>" style="display: block; width: 100%; text-align: center; background: #4f46e5; border-color: #4f46e5; color: #fff;">
        ...
        <div class="sab-panel">
            <div class="sab-panel-header">
                <h2 class="sab-panel-title">🖼️ Existing Featured Images List</h2>
            </div>
            
            <?php if ( empty($display_posts) ): ?>
            <div class="sab-empty-state">No posts with thumbnails found yet.</div>
            <?php else: ?>
            <table class="sab-table">
                ...
                <tbody>
                    ...
                    ?>
                    <tr id="sab-thumb-row-<?php echo (int) $p->ID; ?>">
                        <td class="sab-thumb-preview-cell">
